...
public class BuildingController {
	...
	@PostMapping("/saveAll")
	public ResponseEntity<ApiResponseDto> addMultiBuilding(@RequestBody List<BuildingDto> buildingDto) {
		var response = new ApiResponseDto("1200", "Success",buildingService.createMultiBuilding(buildingDto));
		return new ResponseEntity<>(response, HttpStatus.ACCEPTED);
	}
    @PutMapping("/{buildingId}")
    public ResponseEntity<ApiResponseDto> updateBuilding(@PathVariable("buildingId") int buildingId,
            @RequestBody BuildingDto building) {
        var response = new ApiResponseDto("1200", "Success",buildingService.updateBuilding(buildingId, building));
        return new ResponseEntity<>(response, HttpStatus.ACCEPTED);
    }
}
